...
def _decide_name_order_via_llm_cached(hangul_name: str, context: str, openai_client) -> dict:
    ...
    try:
        ...
        # GPT 호출 실패(크레딧 소진 등) 시 안전한 기본값은 도치하지 않고 그대로 두는 것이다.
        # 여기 들어오는 이름은 대부분 일본·중국 등 한국 성씨가 아닌 한글 표기인데, 이런
        # 이름들은 성-이름 순서를 유지하는 게 원칙(_NAME_ORDER_SYSTEM_PROMPT 1번 규칙)이다.
        # 예전엔 2단어 이름을 무조건 REORDER(서양식 도치)해버려서, 일본 저자 이름이
        # GPT 실패 때마다 "슈사쿠, 미야지"처럼 잘못 뒤집히는 문제가 있었다(2026-08 발견).
...
def _is_translation_foreign_original_author(
    ...
    author_info에 이름이 있다는 것만으로는 부족하다 — 엮은이/그린이/감수 등 한국인
    2차 기여자도 번역자 역할이 아니면 author_info에 같이 들어있어서, 자기 자신과
    매칭돼 원저자로 오판되는 문제가 있었다. 실제로 한자·영문 원표기가 발견됐거나
    동아시아 저자로 확인된(=외국인이라는 근거가 있는) 경우에만 인정한다.
    (2026-08 패치: "100년의 질문" 엮은이 충희 사례 — 원저자와 무관한 한국인 엮은이가
    700 1_로 잘못 나오던 버그.)"""
    ...
    return any(
        ...
        # kanji_name/english_name(프로필 크롤링)이 비어 있어도, 500 빌더가 orig_author_en을
        # 콤마로 나눠서 이 저자 몫으로 확정해둔 _roman_for_900(200e "세계 문화 여행:
        # 싱가포르" — Angela Milligan·Tricia Voute, 개별 프로필은 못 찾았지만 책 전체
        # orig_author_en에서 각자 이름을 나눠 받은 경우)도 같은 신뢰도의 신호다. 이게
        # 빠져 있으면 700 지시기호가 "한국인 출생통계에 없는 이름"으로 오판해 0_이 됨.
        and (ai.get("kanji_name") or ai.get("english_name") or ai.get("is_east_asian") or ai.get("_roman_for_900"))
...
def marc700_second_indicator(
    ...
    *,
    ...
    """
    # build_700()이 실제 $a 값을 만들 때 쓰는 것과 동일하게 "필명(본명)" 괄호를 먼저
    # 벗겨서 판정한다 — 안 그러면 "빈센트(김두언)"처럼 원본 그대로는 KNOWN_PEN_NAMES와
    # 정확히 일치하지 않아 화이트리스트가 무력화된다(2026-08 발견).
    raw = (name or "").strip()
    n = personal_name_for_700_field(raw)
    if not n:
        return "1"
    if n in KNOWN_PEN_NAMES:
        return "0"

    # "필명(본명)" 괄호 구조 자체에서 필명 여부를 추론한다: 괄호 밖(대외 표기)이 로마자·
    # 조어라 애초에 한글 이름 형태가 아닌데, 괄호 안(본명)은 등록 성씨+이름 형태로
    # 확실하면 → 괄호 밖은 필명으로 본다. 예: "UNA(최윤하)", "321web(미쓰이 마사유키)".
    # 괄호 밖이 한글이면(예: "윤홍준(윤샘)", "수지(허수정)") 짧은 1글자 성 조합 등이
    # korean_personal_name_is_structured에서 기술적으로 탈락하는 경우가 있어(예:
    # "수지"=수+지, 3자 미만 컷오프) 오탐 위험이 크므로 이 규칙을 적용하지 않는다
    # — 한글끼리는 기존 로직/화이트리스트로만 판단한다.
    _paren_m = re.match(r"^(.+?)\s*[(（]\s*([^)）]+)\s*[)）]\s*$", raw)
    if _paren_m:
        _inner = _paren_m.group(2).strip()
        if (
            not re.search(r"[가-힣]", n)
            and re.fullmatch(r"[가-힣·ㆍ\s]{2,24}", _inner)
            and korean_personal_name_is_structured(_inner)
        ):
            return "0"
    ...
    if re.search(r"[가-힣]", n):
        ...
        # author_info 대조로 원저자임이 이미 확인된 경우(_is_translation_foreign_original_author)는
        # 그 자체로 신뢰할 수 있는 신호이므로 무조건 1_로 확정한다. "데이비드 J. 건컬"처럼 한글
        # 표기 안에 로마자 이니셜(J)이 섞여 있으면 아래 korean_personal_name_is_structured가
        # 실패해서 엉뚱하게 0_(필명)으로 빠지던 버그(2026-08 발견)의 근본 원인이 바로 이 뒤에
        # 붙어있던 "로마자 글자가 없어야 함" 조건이었다 — 신뢰도 낮은 is_foreign_translator
        # 신호에만 그 조건을 그대로 남겨둔다.
        if _is_translation_foreign_original_author(n, translation_book, ai):
            return "1"
        if is_foreign_translator and not re.search(r"[A-Za-z぀-ヿ]", n):
            return "1"
...
def _author_info_has_japanese_script(author_info: list[dict]) -> bool:
    ...
    단, 그 kanji_name의 주인이 한국인(성씨로 판별)이면 "원저 원어 표기"가 아니라 그냥
    본인 이름의 한자 표기일 뿐이므로 신호에서 제외한다("케임브리지 몽골 제국사" — 서양인
    저자 8명 사이에 한국인 엮은이 김호동 한 명만 있는데, 그의 한자 "金浩東" 때문에 책
    전체가 동아시아 원저로 오판되어 500 원저자명이 통째로 사라지던 버그, 2026-08 발견)."""
    for ai in author_info:
        kn = (ai.get("kanji_name") or "").strip()
        if kn and re.search(r"[぀-ヿ一-鿿㐀-䶿]", kn) and not is_korean_person_by_surname(ai.get("name", "")):
            return True
    return False


# ─────────────────────────────────────────────
# (2026-07 패치) 한자+가나 혼합 원저자명 → 900 한글 표기
#
# 국립중앙도서관 실제 MARC 레코드 대조 결과, 한자 구간은 한국 한자음으로,
# 가나 구간은 발음 그대로 음역하여 구간별로 이어붙이는 방식이 실제 국중 관례와
# 정확히 일치함을 확인했다(예: 上野そら → 상야소라, 住野よる → 주야요루).
# 기존 _jp_name_reading_hangul()(pykakasi로 한자까지 일본어 훈독으로 추측)은
# 이 관례와 다른 결과를 냈다(예: 上野そら → 우에노소라, 오답).
# ─────────────────────────────────────────────
...
def build_900(
    ...
    if japanese_family_book:
        ...
        if kn:
            gloss = _jp_mixed_name_reading_hangul(kn)
            if gloss:
                g0 = re.sub(r"\s+", "", gloss)
                k0 = re.sub(r"\s+", "", ko)
                if g0 != k0:
    ...
    if kanji_name:
        ktype = kanji_type(kanji_name)
        if ktype == "kanji_kana":
            gloss = _jp_mixed_name_reading_hangul(kanji_name)
            if gloss:
                return f"900 10 $a {gloss}"
            return None
        if ktype == "kanji_only":
...
def build_500_700_710_900(ctx: dict, *, openai_client=None) -> dict:
    ...
    # 041 $h(원저작 언어)가 이미 동아시아 언어로 확정된 경우, 성씨 사전에 없는 저자도
    # 원저자로 신뢰할 수 있다는 신호로 쓴다. ("봄은 마차를 타고" — 나쓰메 소세키·아쿠타가와
    # 류노스케 등은 워낙 유명한데도 이 프로젝트의 JAPANESE_SURNAMES 사전엔 없어서
    # is_east_asian이 전부 False로 남았고, kanji_name도 못 찾은 사람은 700 지시기호가
    # 0(필명 취급)으로, 대표저자는 서양식 도치로 잘못 나오던 버그, 2026-08 발견.)
    lang_h = ctx.get("lang_h")
    if lang_h in ("jpn", "chi", "zho"):
        for ai in author_info:
            ai["is_east_asian"] = True
    ...
    # kanji_name이 있어도 그 사람이 한국인(성씨로 판별)이면 "원저자 원어 표기"가 아니라
    # 그냥 한국인 본인 이름의 한자 표기일 뿐이다(예: 편저서의 한국인 엮은이) — 이런 건
    # missing 취급해야 로마자 폴백(orig_author_en)이 정상 작동한다.
    # ("케임브리지 몽골 제국사" — 서양인 저자 8명은 kanji 없음, 한국인 엮은이(김호동)만
    # 한자표기가 있어서 500 원저자명이 로사비 대신 "金浩東"으로 나오던 버그, 2026-08 발견)
    n_missing_kanji = sum(
        1 for x in author_info
        if not (x.get("kanji_name") and not is_korean_person_by_surname(x.get("name", "")))
    )
    ...
    if orig_names_500_from_intro is not None:
        ...
        orig_names_500 = []
        for idx, (ai, tok) in enumerate(zip(author_info, orig_names_500_from_profiles)):
            if _looks_like_roman_orig_display(tok):
                orig_names_500.append(tok)
                ai["_500_script"] = "roman"
                ai["_roman_for_900"] = tok
            elif (
                idx == 0
                and orig_author_en
                and "," not in orig_author_en
                and _looks_like_roman_orig_display(orig_author_en)
            ):
                # 이 저자(보통 1저자)만 프로필 크롤링에서 영문명을 못 찾아서
                # try_orig_names_500_from_profile_english()가 한글 표기로 폴백시켰지만,
                # collect_orig_info()가 GPT 등으로 이 저자의 원저자 로마자 표기를
                # 이미 따로 찾아둔 경우(orig_author_en) 그걸 대신 사용한다.
                # (2026-07 패치: "오디세이아" 니콜라 친퀘티 사례 — 700엔 나오는데
                # 500/900에서만 통째로 빠지던 버그)
                orig_names_500.append(orig_author_en.strip())
                ai["_500_script"] = "roman"
                ai["_roman_for_900"] = orig_author_en.strip()
            else:
        ...
        for ai in author_info:
            ...
            if kanji and not is_korean_person_by_surname(ai.get("name", "")):
                ...
                if (
                    ...
                    for idx, ai in enumerate(author_info):
                        if idx == 0:
                            ...
                            en_prof = (ai.get("english_name") or "").strip()
                            if en_prof and _looks_like_roman_orig_display(en_prof):
                                # 이 저자는 이름 필드(name)는 한글 음역뿐이지만, 프로필
                                # 크롤링으로 찾아둔 영문 원표기(english_name)가 따로 있음
                                # (700은 이미 이 값을 씀 — en_prof 참고). 500/900도 이걸
                                # 활용하면 한글 이름(name)을 그대로 역순 변환해 정확한
                                # 900도 같이 만들 수 있다.
                                # (2026-07 패치: "올라야" Algernon Blackwood 사례 — 700엔
                                # 나오는데 500/900에서 통째로 빠지던 버그)
                                orig_names_500.append(en_prof)
                                ai["_500_script"] = "roman"
                                ai["_roman_for_900"] = en_prof
                            elif not is_korean_person_by_surname(ai["name"].strip()):
                                ai["_500_script"] = "roman"
                            else:
                                ai["_500_script"] = "orig_korean_700_only"
                else:
            ...
            if ai.get("_500_script") == "cjk":
                ...
                for kn_part in kn_parts:
                    ...
                    if ktype == "kanji_only":
                        ...
                        # 한자·가나 혼합 또는 가나만 있는 일본 이름(예: 上野そら, 秋谷りんこ)
                        # → 국중 관례대로 한자 구간은 한국 한자음, 가나 구간은 발음 그대로
                        # 음역해 이어붙임(2026-07 패치, 국중 실제 레코드 대조로 확인).
