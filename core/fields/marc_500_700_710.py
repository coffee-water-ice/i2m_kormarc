                ...
                if (
                    ...
                    for idx, ai in enumerate(author_info):
                        if idx == 0:
                            ...
                            nm = ai["name"].strip()
                            if _looks_like_roman_orig_display(nm):
                                # 이 저자는 이름 필드 자체가 이미 로마자 원표기임(별도 한글
                                # 표기 없이 원서에서부터 로마자로만 실린 공저자 등). 500엔
                                # 표기하되, 한글 표기 원본이 없어 900은 생성하지 않는다.
                                # (2026-07 패치: "올라야" Algernon Blackwood 사례 — 700엔
                                # 나오는데 500에서 통째로 빠지던 버그)
                                orig_names_500.append(nm)
                                ai["_500_script"] = "roman_500_only"
                            elif not is_korean_person_by_surname(nm):
                                ai["_500_script"] = "roman"
                            else:
                                ai["_500_script"] = "orig_korean_700_only"
                else:
