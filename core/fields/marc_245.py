...
def _responsibility_pairs_from_authors(authors: list[dict]) -> list[tuple[str, str]]:
    ...
    단, 개인 저자가 하나도 없고 저자 전원이 기관이면 기관 저자로 대체한다 —
    안 그러면 245 $d가 통째로 비어버린다(2026-08 발견: "유물멍" 사례, 저자가
    국립중앙박물관 하나뿐인데 $d에 아무것도 안 붙던 문제)."""
    pairs: list[tuple[str, str]] = []
    org_pairs: list[tuple[str, str]] = []
    for a in authors:
        ...
        if a.get("is_org"):
            org_pairs.append((role, name))
            continue
        pairs.append((role, name))
    return pairs or org_pairs
