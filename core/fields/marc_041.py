...
이식 시 적용한 원칙 (docs/INTEGRATION_PRINCIPLES.md):
  ...
  - #8  Optional[X] → X | None 등 최신 타입힌트 스타일로 정리.
...
def _resolve_lang_name_to_isds(lang_name: str) -> str | None:
    """
...
def infer_language_from_major_text(major: str | None) -> str | None:
...
def extract_univ_major_regex(text: str) -> dict[str, str | None] | None:
...
class LangFieldBuilder:
    ...
    def __init__(
        ...
        openai_client=None,
        model: str = "gpt-4o",
        dbg_fn:     Callable | None = None,
        dbg_err_fn: Callable | None = None,
    ):
...
""".strip()
    ...
    @staticmethod
    def detect_language_from_category(text: str) -> str | None:
    ...
    @staticmethod
    def _trigger_kw(tokens: list[str], kws: list[str]) -> str | None:
        s = set(tokens)
        for k in kws:
            if k in s:
                return k
    ...
    def determine_h_language(
        ...
        author: str,
        subject_lang: str,
        item: dict | None = None,  # API item dict 전체 (Bio 수집에 사용)
    ...
    def get_kormarc_tags(
        ...
        (tag_041, tag_546, original_title)
          ...
          - 예외 발생 시        : (None, None, original_title) — 원본은 여기서 예외 메시지
            문자열을 tag_041 자리에 섞어 반환하는 안티패턴이 있었다(원칙 #6 위반) — 고쳤다.
            에러 내용은 dbg_err로 남는다.
        """
        item   = item   or {}
        detail = detail or {}
    ...
    @staticmethod
    def as_mrk_041(tag_041: str | None) -> str | None:
        """
    ...
    @staticmethod
    def as_mrk_546(tag_546_text: str | None) -> str | None:
        """
    ...
    @staticmethod
    def lang3_from_tag041(tag_041: str | None) -> str | None:
