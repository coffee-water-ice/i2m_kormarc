...
KNOWN_PEN_NAMES = frozenset({"하와이 대저택", "다숲", "느린호수", "빈센트", "윤샘", "난다유"})
...
_SPANISH_GIVEN_NAMES_LATIN = frozenset({
    ...
    # 남성 (확장) — "Alonso"는 제외: 이름보다 복성 일부(예: Mario Alonso Puig)로 훨씬
    # 흔히 쓰여서, 목록에 넣으면 복성 판별이 깨짐(2026-08 발견, 회귀 확인 후 제거).
    "Ángel", "Angel", "Emiliano", "Esteban", "Ezequiel", "Fabián", "Fabian",
...
def is_org(name: str) -> bool:
    ...
    # 단, 공백으로 나뉜 "성 이름" 형태(예: "구루마다 가즈히사")는 실제 인명 표기이지 출판사명이
    # 아니다 — 이름 끝 음절이 우연히 "사"로 끝나는 일본인 이름(가즈히사 등)까지 기관으로
    # 오판되던 문제(2026-08 발견). 알라딘 표기에서 출판사·기관명은 공백 없는 한 단어로,
    # 인명은 "성 이름" 공백 구분으로 나오는 관례를 이용해 구분한다.
    hangul_syllables = [c for c in n if "가" <= c <= "힣"]
    if len(hangul_syllables) >= 3 and n.endswith("사") and " " not in n:
        return True
    return False
