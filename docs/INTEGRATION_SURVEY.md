...
> **통합 현황**: 아래 조사는 통합 착수 시점의 원본 4개 폴더를 스냅샷으로 분석한
> 것이며, 041/245/653/260+300 전부 `i2m_kormarc/`로 이식이 완료되어 실제 동작한다.
> 현재 이식 상태는 `i2m_kormarc/README.md`의 "현재 상태"를 참고할 것 — 이 문서는
> 각 폴더의 원래 설계·구조를 이해하기 위한 참고 자료로 유지한다.
