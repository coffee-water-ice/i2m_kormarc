...
|---|---|---|---|---|---|
| model8~10 | 384 | 60 | 150 | 100 | `prepare_data_v8.py` |
| **model11~12** | **512** | 60 | **200** | **140** | `prepare_data_v11_longctx.py` |

model12 전용 전처리 스크립트는 없다 — `prepare_data_v11_longctx.py`를 그대로 쓴다
(딥러닝팀 확인). 구분자(`" [SEP] "`)와 필드 순서(본표제 → 카테고리 → 653키워드 →
목차 → 책소개)는 model8~12에서 바뀌지 않았다.
